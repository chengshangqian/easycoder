...
<xsl:text>  </xsl:text>FROM <xsl:value-of select="metadata/tables/table/upperTableName" /> WHERE IS_DELETED = 'N'
<xsl:text>  </xsl:text>
<dynamic prepend="">
<xsl:for-each select="metadata/tables/table/columns/column">
...
<xsl:text>  </xsl:text>UPDATE <xsl:value-of select="metadata/tables/table/upperTableName" /> SET IS_DELETED = #isDeleted# WHERE <xsl:value-of select="$primaryColumnKey" /> = #<xsl:value-of select="$primaryLowerKey" />#
<xsl:text>  </xsl:text></xsl:element>
