...
public class ${MROUtil.tableNameToClassName(metadata.tables[0].tableName)}Action extends BaseAction{
	...
	public String findByPage() throws Exception{
	    ...
	    String ${MROUtil.tableNameToBeanName(column.columnName)} = this.formMap.get("${MROUtil.tableNameToBeanName(column.columnName)}");
	    if(null != ${MROUtil.tableNameToBeanName(column.columnName)} && !"".equals(${MROUtil.tableNameToBeanName(column.columnName)}.trim())){
		    this.resultMap.put("${MROUtil.tableNameToBeanName(column.columnName)}", AppUtil.toDate(${MROUtil.tableNameToBeanName(column.columnName)}));
		}
	    </#if>
	    </#list>	
	...
	public String create(){
		this.${MROUtil.tableNameToBeanName(metadata.tables[0].tableName)}Service.create(this.formMap);
		return SUCCESS;
	}
	...
	public String modify(){
		this.${MROUtil.tableNameToBeanName(metadata.tables[0].tableName)}Service.modify(this.formMap);
		return SUCCESS;
	}
	...
	public String remove(){
		this.${MROUtil.tableNameToBeanName(metadata.tables[0].tableName)}Service.remove(this.formMap);
		return SUCCESS;
	}
}	