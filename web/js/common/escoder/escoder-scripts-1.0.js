 ...
 * Date: 2011/11/2
 */
if ( "undefined" == typeof(ESCoder) || !ESCoder ) {
	var ESCoder = {};
}

ESCoder.Script = {
	loadScript: function(url, onload) {
		ESCoder.Script.loadScriptDomElement(url, onload);
	},
	...
	loadScripts: function(aUrls, onload) {
		...
		for ( var i = 0; i < nUrls; i++ ) {
			if ( ESCoder.Script.differentDomain(aUrls[i]) ) {
				bDifferent = true;
				break;
			}
		}
		...
		var loadFunc = ESCoder.Script.loadScriptXhrInjection;
		if ( bDifferent ) {
			if ( -1 != navigator.userAgent.indexOf('Firefox') || 
				 -1 != navigator.userAgent.indexOf('Opera') ) {
				loadFunc = ESCoder.Script.loadScriptDomElement;
			}
			else {
				loadFunc = ESCoder.Script.loadScriptDocWrite;
			}
		}
	...
	loadScriptDocWrite: function(url, onload) {
		...
		if ( onload ) {
			...
			ESCoder.addHandler(window, "load", onload);
		}
	},
	...
	loadScriptXhrInjection: function(url, onload, bOrder) {
		var iQueue = ESCoder.Script.queuedScripts.length;
		if ( bOrder ) {
			var qScript = { response: null, onload: onload, done: false };
			ESCoder.Script.queuedScripts[iQueue] = qScript;
		}

		var xhrObj = ESCoder.Script.getXHRObject();
		xhrObj.onreadystatechange = function() { 
			if ( xhrObj.readyState == 4 ) {
				if ( bOrder ) {
					ESCoder.Script.queuedScripts[iQueue].response = xhrObj.responseText;
					ESCoder.Script.injectScripts();
				}
				else {
	...
	injectScripts: function() {
		var len = ESCoder.Script.queuedScripts.length;
		for ( var i = 0; i < len; i++ ) {
			var qScript = ESCoder.Script.queuedScripts[i];
			if ( ! qScript.done ) {
				if ( ! qScript.response ) {
...
ESCoder.addHandler = function(elem, type, func) {
